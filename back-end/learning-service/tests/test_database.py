...
existente cobria a diferença: `app/scheduler.py` e `app/events/consumer.py`
sempre trocam `async_session` por `test_session_factory` via `monkeypatch`
(ver `tests/test_scheduler.py`, `tests/test_consumer.py`), e `get_db` é
sempre sobrescrito por `_override_get_db` na fixture `client` — o objeto real
exportado por `app/database.py` nunca era exercitado sem substituição.
