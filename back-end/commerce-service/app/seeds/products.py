...
aa98694 ("feat(marketplace): load real products and reviews from the API"), and
`grep -rl "Guia de Redação Nota 1000" front-end-flutter/` returns nothing. Its
last version (`git show aa98694^:front-end-flutter/lib/features/marketplace/
data/mock_marketplace.dart`) did carry these same six names/types/subtypes/
prices/ratings — the claim was true, it is just no longer checkable against a
live file, so it is recorded here as history rather than as a live invariant.
...
Run inside the commerce-service container (`make services-seed`):
